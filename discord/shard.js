...
client.on("guildCreate", function (guild) {
	...
	guild.owner.send("SupportBot has joined your server: " + guild.name + "\nUse `Lhelp` for information on how to use SupportBot.\nAdd SupportBot to other servers using this link: <" + CONFIG.BOT_ADD_LINK + "> . SupportBot is a work in progress! Help us improve supportbot by sending us your feedback at " + CONFIG.HELP_SERVER_INVITE_LINK).catch(e => console.error(e));
	let candidate = UTILS.preferredTextChannel(client, guild.channels, "text", UTILS.defaultChannelNames(), ["VIEW_CHANNEL", "SEND_MESSAGES"]);
	if (UTILS.exists(candidate)) candidate.send("Use `Lhelp` for information on how to use SupportBot.\nAdd SupportBot to other servers using this link: <" + CONFIG.BOT_ADD_LINK + "> . SupportBot is a work in progress! Help us improve supportbot by sending us your feedback at " + CONFIG.HELP_SERVER_INVITE_LINK).catch();
});
