...
module.exports = class EmbedGenrator {
	...
	detailedMatch(CONFIG, summoner, match_meta, match) {//should show detailed information about 1 game
		...
		for (let b in teams) {
			...
			newEmbed.addField((match.teams.find(t => { return teams[b][0].teamId == t.teamId; }).win == "Win" ? "<:win:409617613161758741>" : "<:loss:409618158165688320>") + "Team " + team_count, "Σlv.`" + teams[b].reduce((total, increment) => { return total + increment.stats.champLevel; }, 0) + "`\t`" + tK + "/" + tD + "/" + tA + "`\tKDA:`" + (UTILS.round(((tK + tA) / tD), 2) == "Infinity" ? "Perfect" : UTILS.round(((tK + tA) / tD), 2)) + "` `" + tKP + "%`\tΣcs:`" + teams[b].reduce((total, increment) => { return total + increment.stats.totalMinionsKilled + increment.stats.neutralMinionsKilled; }, 0) + "`\tΣg:`" + UTILS.gold(teams[b].reduce((total, increment) => { return total + increment.stats.goldEarned; }, 0)) + "`");
			for (let c in teams[b]) {
				let p = teams[b][c];
				newEmbed.addField("__" + match.participantIdentities.find(pI => { return pI.participantId == p.participantId; }).player.summonerName + "__: " + CONFIG.STATIC.CHAMPIONS[p.championId].name + " " + (UTILS.english(p.timeline.role) == "None" ? "" : UTILS.english(p.timeline.role)) + " " + UTILS.english(p.timeline.lane), "lv. `" + p.stats.champLevel + "`\t`" + p.stats.kills + "/" + p.stats.deaths + "/" + p.stats.assists + "`\tKDA:`" + (UTILS.round(((p.stats.kills + p.stats.assists) / p.stats.deaths), 2) == "Infinity" ? "Perfect" : UTILS.round(((p.stats.kills + p.stats.assists) / p.stats.deaths), 2)) + "` `" + UTILS.round((100 * (p.stats.assists + p.stats.kills)) / tK, 0) + "%`\tcs:`" + (p.stats.totalMinionsKilled + p.stats.neutralMinionsKilled) + "`\tg:`" + UTILS.gold(p.stats.goldEarned) + "`");
			}
		}
		// champion
		// match result
